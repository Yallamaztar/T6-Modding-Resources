...
CoD.OptionsControls.CreateLookTab = function (f6_arg0, f6_arg1)
	local f6_local0 = LUI.UIContainer.new()
	local f6_local1 = CoD.Options.CreateButtonList()
	f6_arg0.buttonList = f6_local1
	f6_local0:addElement(f6_local1)
	CoD.OptionsControls.AddKeyBindingElements(f6_arg1, f6_local1, {
		{
			command = "+leanleft",
	...
	local f6_local2 = f6_local1:addProfileLeftRightSlider(f6_arg1, Engine.Localize("MENU_MOUSE_SENSITIVITY_CAPS"), "mouseSensitivity", 0.01, 30, nil, nil, nil, CoD.Options.AdjustSFX)
...
CoD.OptionsControls.TabChanged = function (HudRef, ClientInstance)
	HudRef.buttonList = HudRef.tabManager.buttonList
	local Button = HudRef.buttonList:getFirstChild()
	while not Button.m_focusable do
	...
	CoD.OptionsControls.CurrentTabIndex = ClientInstance.tabIndex
end
