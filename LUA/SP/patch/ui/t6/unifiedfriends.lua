...
CoD.FriendsListPopup.UpdatePromptButtonVis = function (f10_arg0)
	...
	if CoD.FriendsListPopup.Mode == CoD.playerListType.facebook and nil ~= Engine.IsFacebookLinked and Engine.IsFacebookLinked(f10_local0) == false and 0 == UIExpression.IsInGame() then
...
CoD.FriendsListPopup.RefreshList = function (self, f13_arg1)
	local f13_local0 = self.listBox:getFocussedMutables()
	local f13_local1 = {
		index = 1
	}
	local f13_local2 = {}
	...
	if self ~= nil and self.listBox ~= nil then
		if f13_local0 ~= nil and f13_local0.playerXuid ~= nil and f13_local0.playerXuid ~= 0 then
			f13_local1 = Engine.GetPlayerIndexByXuid(self.m_ownerController, f13_local0.playerXuid, f13_local2)
		...
		if CoD.FriendsListPopup.Mode == CoD.playerListType.facebook then
			if nil ~= Engine.IsFacebookLinked and Engine.IsFacebookLinked(controller) then
				if nil ~= Engine.IsFacebookDuplicate and Engine.IsFacebookDuplicate(controller) then
					self.listBox.noDataText = Engine.Localize("MENU_FB_ACCOUNT_IN_USE")
				else
					self.listBox.noDataText = Engine.Localize("MENU_FRIENDS_FB_NONE")
				end
			else
				self.listBox.noDataText = Engine.Localize("MENU_FRIENDS_FB_NOT_LINKED")
			end
		end
		self.listBox:setTotalItems(Engine.GetPlayerCount(self.m_ownerController, CoD.FriendsListPopup.Mode), f13_local1.index)
		CoD.FriendsListPopup.UpdatePromptButtonVis(self)
		self.listBox:refresh()
		if Engine.GetPlayerCount(self.m_ownerController, CoD.FriendsListPopup.Mode) == 0 and CoD.FriendDetails ~= nil then
			CoD.FriendDetails.hide(self.details)
		end
	end
end
...
CoD.FriendsListPopup.PlayerSignedOut = function (f27_arg0, f27_arg1)
	if f27_arg0.m_ownerController == f27_arg1.controller then
		local f27_local0 = f27_arg0.occludedMenu
		f27_arg0:dispatchEventToRoot({
			name = "close_all_popups",
			controller = f27_arg1.controller
		})
		local f27_local1 = f27_local0:openPopup("Error", f27_arg1.controller)
		f27_local1.anyControllerAllowed = true
		f27_local1:setMessage(Engine.Localize("XBOXLIVE_SIGNINCHANGED"))
	end
end

CoD.FriendsListPopup.FacebookLink = function (f28_arg0, f28_arg1)
	if Engine.IsFacebookLinked ~= nil and Engine.IsFacebookLinked(f28_arg1.controller) == false then
		Engine.Exec(f28_arg1.controller, "fbLinkFacebook")
	end
end

LUI.createMenu.FriendsList = function (f29_arg0, f29_arg1)
	Engine.Exec(f29_arg0, "updateInfoForInGameList")
	local f29_local0 = CoD.Menu.New("FriendsList")
	if not Engine.IsInGame() then
		f29_local0:addLargePopupBackground()
	end
	if f29_arg1 ~= nil and f29_arg1.previousMenu ~= nil then
		f29_local0:setPreviousMenu(f29_arg1.previousMenu)
		f29_local0.previousMenu = f29_arg1.previousMenu
	end
	f29_local0:setOwner(f29_arg0)
	f29_local0:registerEventHandler("close_friends_list", CoD.FriendsList.CloseMenu)
	...
	f29_local0:registerEventHandler("probation_confirmation", CoD.FriendsListPopup.FinishJoinSession)
	...
	f29_local0:registerEventHandler("xboxlive_party", CoD.FriendsListPopup.XboxLIVEParty)
	f29_local0:registerEventHandler("ps3_invitations", CoD.FriendsListPopup.PS3Invitations)
	...
	f29_local0:registerEventHandler("signed_out", CoD.FriendsListPopup.PlayerSignedOut)
	f29_local0:registerEventHandler("fb_linkFacebook", CoD.FriendsListPopup.FacebookLink)
	if CoD.useMouse then
		f29_local0:registerEventHandler("listbox_clicked", CoD.FriendsListPopup.FriendSelected)
	end
	if CoD.isWIIU then
		f29_local0:registerEventHandler("open_saveandquitgamepopup", CoD.FriendsListPopup.SaveAndQuit)
		f29_local0:registerEventHandler("closeFriendsList", CoD.FriendsListPopup.Close)
		if not Engine.IsInGame() then
			LUI.roots.UIRootFull.friendsMenu = f29_local0
		else
			local f29_local1 = LUI.roots
			local f29_local2 = "UIRoot"
			f29_local1["UIRoot" .. f29_arg0].friendsMenu = f29_local0
		end
	end
	f29_local0.selectButton = CoD.ButtonPrompt.new("primary", Engine.Localize("MENU_SELECT"), f29_local0, "friend_selected")
	local f29_local1
	if CoD.isZombie then
		f29_local1 = not Engine.IsInGame()
	else
		f29_local1 = true
	end
	if f29_local1 == true then
		local f29_local2 = CoD.ButtonPrompt.new
		local f29_local3 = "select"
		local f29_local4 = Engine.Localize("MENU_INVITE_GAME")
		local f29_local5 = f29_local0
		local f29_local6 = "friend_invitetosession"
		local f29_local7, f29_local8 = false
		local f29_local9, f29_local10 = false
		f29_local0.inviteButton = f29_local2(f29_local3, f29_local4, f29_local5, f29_local6, f29_local7, f29_local8, f29_local9, f29_local10, "I")
		f29_local2 = CoD.ButtonPrompt.new
		f29_local3 = "alt1"
		f29_local4 = Engine.Localize("MENU_JOIN_IN_PROGRESS")
		f29_local5 = f29_local0
		f29_local6 = "friend_joinsession"
		f29_local7, f29_local8 = false
		f29_local9, f29_local10 = false
		f29_local0.joinButton = f29_local2(f29_local3, f29_local4, f29_local5, f29_local6, f29_local7, f29_local8, f29_local9, f29_local10, "J")
	end
	f29_local0.backButton = CoD.ButtonPrompt.new("secondary", Engine.Localize("MPUI_DONE"), f29_local0, "close_friends_list")
	f29_local0.partyButton = CoD.ButtonPrompt.new("start", Engine.Localize("MENU_XBOXLIVE_PARTY"), f29_local0, "xboxlive_party")
	f29_local0.linkFBButton = CoD.ButtonPrompt.new("alt1", Engine.Localize("MENU_FB_LINK"), f29_local0, "fb_linkFacebook")
	if CoD.isPS3 then
		f29_local0.invitationsButton = CoD.ButtonPrompt.new("start", Engine.Localize("PLATFORM_VIEW_GAME_INVITES"), f29_local0, "ps3_invitations")
	end
	if not CoD.isSinglePlayer then
	...
	}, f29_arg0, 7, CoD.CoD9Button.Height * 2, 400, f0_local0, f0_local1, 5, 2)
	CoD.ListBox.HideMessage(f29_local0.listBox, true)
	local f29_local4 = {
		left = 0,
		top = 70,
		right = 0,
	...
	local f29_local5 = CoD.MFTabManager.new(f29_local0.tabContentPane, f29_local4)
	f29_local5:keepRightBumperAlignedToHeader(true)
	f29_local5.popup = f29_local0
	f29_local0.tabManager = f29_local5
	local f29_local6 = "Big"
	local f29_local7 = CoD.fonts[f29_local6]
	local f29_local8 = CoD.textSize[f29_local6]
	f29_local0.header = LUI.UIText.new()
	f29_local0.header:setLeftRight(true, true, 0, 0)
	f29_local0.header:setTopBottom(true, false, 15, 15 + f29_local8)
	f29_local0.header:setRGB(CoD.offWhite.r, CoD.offWhite.g, CoD.offWhite.b)
	f29_local0.header:setFont(f29_local7)
	f29_local0.header:setAlignment(LUI.Alignment.Left)
	if CoD.isMultiplayer and not CoD.isZombie then
		f29_local0:addElement(CoD.MiniIdentity.GetMiniIdentity(f29_arg0))
	end
	f29_local0.listBox.m_positionTextString = "MENU_LB_LISTBOX_POSITION_TEXT"
	f29_local0.listBox:addScrollBar(30, 2)
	f29_local0.listBox.m_pageArrowsOn = true
	f29_local0:addElement(f29_local0.header)
	f29_local0:addElement(f29_local5)
	f29_local0:addElement(f29_local0.tabContentPane)
	f29_local0.joinText = LUI.UIText.new()
	f29_local0:addElement(f29_local0.joinText)
	f29_local0.joinText:setText("")
	f29_local5:addTab(f29_arg0, "MENU_TAB_FRIENDS_CAPS", f0_local2)
	f29_local5:addTab(f29_arg0, "MENU_TAB_LEAGUE_FRIENDS_CAPS", f0_local3)
	if not CoD.isPC then
		f29_local5:addTab(f29_arg0, "MENU_TAB_FACEBOOK_CAPS", f0_local4)
	end
	if Engine.IsEliteAvailable() then
		f29_local5:addTab(f29_arg0, "MENU_TAB_ELITE_CLAN_CAPS", f0_local6)
	end
	f29_local5:addTab(f29_arg0, "MENU_TAB_PLAYERS_CAPS", f0_local5)
	if CoD.isXBOX then
		f29_local5:addTab(f29_arg0, "MENU_TAB_XBOXLIVE_PARTY", f0_local7)
	end
	if CoD.isWIIU or CoD.isPC then
		f29_local5:addTab(f29_arg0, "MENU_GAME_INVITES_CAPS", f0_local8)
	end
	if CoD.isWIIU then
		f29_local5:addTab(f29_arg0, "PLATFORM_FRIEND_REQUESTS_CAPS", f0_local9)
	end
	if CoD.isWIIU or nil ~= LUI.roots.UIRootDrc then
		f29_local0.textMessageTabIndex = f29_local5:addTab(f29_arg0, "PLATFORM_UI_TM_TEXT_MESSAGES_CAPS", f0_local10)
	end
	f29_local5:refreshTab(f29_arg0)
	f29_local0:addElement(f29_local0.listBox)
	f29_local0.listBox:setTotalItems(Engine.GetPlayerCount(f29_arg0, CoD.FriendsListPopup.Mode), 1)
...
LUI.createMenu.IngameFriendsList = function (f30_arg0)
	local f30_local0 = nil
	f30_local0 = CoD.Menu.New("FriendsList")
	f30_local0:addLargePopupBackground()
	f30_local0:setOwner(f30_arg0)
	f30_local0:setPreviousMenu("class")
	f30_local0:registerEventHandler("friend_invitetosession", CoD.FriendsListPopup.InviteToSession)
	f30_local0:registerEventHandler("friend_joinsession", CoD.FriendsListPopup.JoinSession)
	f30_local0:registerEventHandler("listbox_focus_changed", CoD.FriendsListPopup.FriendFocusChanged)
	f30_local0:registerEventHandler("friend_selected", CoD.FriendsListPopup.FriendSelected)
	f30_local0:registerEventHandler("friends_updated", CoD.FriendsListPopup.RefreshList)
	f30_local0:registerEventHandler("closeallpopups", CoD.FriendPopup.Close)
	f30_local0:registerEventHandler("close_all_ingame_menus", CoD.FriendPopup.Close)
	if CoD.useMouse then
		f30_local0:registerEventHandler("listbox_clicked", CoD.FriendsListPopup.FriendSelected)
	end
	f30_local0.selectButton = CoD.ButtonPrompt.new("primary", Engine.Localize("MENU_SELECT"), f30_local0, "friend_selected")
	local f30_local1 = CoD.ButtonPrompt.new
	local f30_local2 = "alt2"
	local f30_local3 = Engine.Localize("MENU_INVITE_GAME")
	local f30_local4 = f30_local0
	local f30_local5 = "friend_invitetosession"
	local f30_local6, f30_local7 = false
	local f30_local8, f30_local9 = false
	f30_local0.inviteButton = f30_local1(f30_local2, f30_local3, f30_local4, f30_local5, f30_local6, f30_local7, f30_local8, f30_local9, "I")
	f30_local1 = CoD.ButtonPrompt.new
	f30_local2 = "alt1"
	f30_local3 = Engine.Localize("MENU_JOIN_IN_PROGRESS")
	f30_local4 = f30_local0
	f30_local5 = "friend_joinsession"
	f30_local6, f30_local7 = false
	f30_local8, f30_local9 = false
	f30_local0.joinButton = f30_local1(f30_local2, f30_local3, f30_local4, f30_local5, f30_local6, f30_local7, f30_local8, f30_local9, "J")
	f30_local0.backButton = CoD.ButtonPrompt.new("secondary", Engine.Localize("MPUI_DONE"), f30_local0, "button_prompt_back")
	if not CoD.isSinglePlayer then
		f30_local0.details = CoD.FriendDetails.new()
		f30_local0:addElement(f30_local0.details)
	end
	f30_local0.listBox = CoD.ListBox.new({
		left = 0,
		top = 30,
		right = 0,
		bottom = -CoD.ButtonPrompt.Height,
		...
		rightAnchor = true,
		bottomAnchor = true
	}, f30_arg0, 8, CoD.CoD9Button.Height * 2, 400, f0_local0, f0_local1, 5, 2)
	Dvar.ui_friendsListOpen:set(true)
	f30_local2 = {
		left = 0,
		top = 0,
	...
	f30_local0.tabContentPane = LUI.UIElement.new({
		left = 0,
		top = 0,
		right = 0,
		bottom = -CoD.ButtonPrompt.Height,
	...
	f30_local3 = CoD.MFTabManager.new(f30_local0.tabContentPane, f30_local2)
	f30_local3.popup = f30_local0
	f30_local0.tabManager = f30_local3
	f30_local0:addElement(f30_local3)
	f30_local0:addElement(f30_local0.tabContentPane)
	f30_local3:addTab(f30_arg0, "MENU_TAB_FRIENDS_CAPS", f0_local2)
	f30_local3:addTab(f30_arg0, "MENU_TAB_PLAYERS_CAPS", f0_local5)
	f30_local3:addTab(f30_arg0, "MENU_GAME_INVITES_CAPS", f0_local8)
	f30_local3:refreshTab(f30_arg0)
	f30_local0:addElement(f30_local0.listBox)
	f30_local0.listBox:setTotalItems(Engine.GetPlayerCount(f30_arg0, CoD.FriendsListPopup.Mode))
	CoD.FriendsListPopup.UpdatePromptButtonVis(f30_local0)
	Engine.PlaySound("cac_loadout_edit_sel")
	if CoD.isWIIU then
		f30_local4 = LUI.roots
		f30_local5 = "UIRoot"
		f30_local4["UIRoot" .. f30_arg0].friendsMenu = f30_local0
	end
	return f30_local0
end
