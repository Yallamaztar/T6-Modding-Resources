...
CoD.StickLayout.SelectionChanged = function (f2_arg0)
...
CoD.StickLayout.AddChoices = function (f8_arg0)
	local f8_local0 = tonumber(UIExpression.ProfileValueAsString(f8_arg0.m_currentController, "input_invertpitch"))
...
local f0_local0 = function (f12_arg0)
	f12_arg0.stickLayoutImageContainer.leftStickImage:setLeftRight(false, false, -120, 0)
	f12_arg0.stickLayoutImageContainer.leftStickImage:setTopBottom(false, false, 90, 210)
	f12_arg0.stickLayoutImageContainer.rightStickImage:setLeftRight(false, false, 0, 120)
	f12_arg0.stickLayoutImageContainer.rightStickImage:setTopBottom(false, false, 90, 210)
	local f12_local0, f12_local1 = Engine.GetUserSafeArea()
	local f12_local2 = CoD.textSize.Condensed
	f12_arg0.stickLayoutImageContainer.textUL:setLeftRight(false, false, -120, 0)
	f12_arg0.stickLayoutImageContainer.textUL:setTopBottom(false, false, 60, 60 + f12_local2)
	f12_arg0.stickLayoutImageContainer.textL:setLeftRight(false, false, -f12_local0 / 2, -130)
	f12_arg0.stickLayoutImageContainer.textL:setTopBottom(false, false, 130, 130 + f12_local2)
	f12_arg0.stickLayoutImageContainer.textL:setAlignment(LUI.Alignment.Right)
	f12_arg0.stickLayoutImageContainer.textLL:setLeftRight(false, false, -120, 0)
	f12_arg0.stickLayoutImageContainer.textLL:setTopBottom(false, false, 215, 215 + f12_local2)
	f12_arg0.stickLayoutImageContainer.textUR:setLeftRight(false, false, 120, 0)
	f12_arg0.stickLayoutImageContainer.textUR:setTopBottom(false, false, 60, 60 + f12_local2)
	f12_arg0.stickLayoutImageContainer.textUR:setAlignment(LUI.Alignment.Center)
	f12_arg0.stickLayoutImageContainer.textLR:setLeftRight(false, false, 120, 0)
	f12_arg0.stickLayoutImageContainer.textLR:setTopBottom(false, false, 215, 215 + f12_local2)
	f12_arg0.stickLayoutImageContainer.textR:setLeftRight(false, false, 130, f12_local0 / 2)
	f12_arg0.stickLayoutImageContainer.textR:setTopBottom(false, false, 130, 130 + f12_local2)
...
CoD.StickLayout.CreateLayout = function (f14_arg0)
	...
	if CoD.isPS3 then
		f14_local0 = "ps3_controller_top"
	elseif CoD.isWIIU then
		local f14_local1 = UIExpression.GetControllerType(controller)
		if f14_local1 == "classic" then
			f14_local0 = "wiiu_ccp_top"
		elseif f14_local1 == "uc" then
			f14_local0 = "wiiu_uc_top"
		else
			f14_local0 = "wiiu_drc_top"
		end
	else
	...
	elseif CoD.isPS3 then
		f0_local0(f14_arg0)
	elseif CoD.isWIIU then
		CoD.WiiUButtonLayout.PlaceStickLayoutElements(f14_arg0, f14_arg0.controller)
	end
end
...
CoD.StickLayout.CloseMenu = function (f15_arg0)
	...
	CoD.Options.Close(f15_arg0)
end

LUI.createMenu.StickLayout = function (f16_arg0, f16_arg1)
	local f16_local0 = nil
	if UIExpression.IsInGame() == 1 then
		f16_local0 = CoD.InGameMenu.New("StickLayout", f16_arg0, Engine.Localize("PLATFORM_STICK_LAYOUT_CAPS"), CoD.isSinglePlayer)
		if CoD.isSinglePlayer == false and UIExpression.IsInGame() == 1 and Engine.IsSplitscreen() == true then
			f16_local0:sizeToSafeArea(f16_arg0)
			f16_local0:updateTitleForSplitscreen()
			f16_local0:updateButtonPromptBarsForSplitscreen()
		end
	else
		f16_local0 = CoD.Menu.New("StickLayout")
		f16_local0:setOwner(f16_arg0)
		f16_local0:addTitle(Engine.Localize("PLATFORM_STICK_LAYOUT_CAPS"))
		if CoD.isSinglePlayer == false then
			f16_local0:addLargePopupBackground()
			f16_local0:registerEventHandler("signed_out", CoD.Options.SignedOut_MPZM)
		end
	end
	f16_local0.close = CoD.StickLayout.CloseMenu
	f16_local0.controller = f16_arg0
	f16_local0:addBackButton()
	f16_local0:registerEventHandler("stick_layout_selection_changed", CoD.StickLayout.StickLayoutSelectionChanged)
	local f16_local1 = 0
	if UIExpression.IsInGame() == 1 and CoD.isSinglePlayer == false and Engine.IsSplitscreen() == true then
		f16_local1 = CoD.Menu.SplitscreenSideOffset
	end
	f16_local0.stickLayoutButtonList = CoD.ButtonList.new({
		leftAnchor = true,
		rightAnchor = false,
		left = f16_local1,
		right = f16_local1 + CoD.ButtonList.DefaultWidth - 20,
		topAnchor = true,
		bottomAnchor = false,
	...
	if CoD.isPC then
		f16_local0.stickLayoutButtonList:setLeftRight(true, false, f16_local1, f16_local1 + 500)
	end
	if CoD.isSinglePlayer then
		f16_local0:addElement(f16_local0.stickLayoutButtonList)
		if f16_arg1 and f16_arg1.height ~= nil then
			f16_local0.stickLayoutButtonList:setLeftRight(false, false, -CoD.Options.Width / 2, CoD.Options.Width / 2)
			f16_local0.stickLayoutButtonList:setTopBottom(false, false, -f16_arg1.height / 2, f16_arg1.height / 2)
		end
	else
		local f16_local2 = CoD.SplitscreenScaler.new(nil, 1.5)
		f16_local2:setLeftRight(true, false, 0, 0)
		f16_local2:setTopBottom(true, false, 0, 0)
		f16_local0:addElement(f16_local2)
		f16_local2:addElement(f16_local0.stickLayoutButtonList)
	end
	f16_local0.stickLayoutSelector = f16_local0.stickLayoutButtonList:addProfileLeftRightSelector(f16_arg0, Engine.Localize("PLATFORM_THUMBSTICK_LAYOUT_CAPS"), "gpad_sticksConfig")
	CoD.StickLayout.AddChoices(f16_local0.stickLayoutSelector)
	CoD.StickLayout.CreateLayout(f16_local0)
	CoD.StickLayout.SetTextElems(f16_local0.stickLayoutImageContainer, f16_local0.stickLayoutSelector.m_choices[f16_local0.stickLayoutSelector.m_currentChoice].params)
	CoD.StickLayout.AddImageElements(f16_local0.stickLayoutImageContainer)
	CoD.StickLayout.AddTextElements(f16_local0.stickLayoutImageContainer)
	for f16_local2 = 1, #f16_local0.stickLayoutSelector.m_choices, 1 do
		f16_local0.stickLayoutImageContainer.leftStickImage:registerAnimationState(f16_local0.stickLayoutSelector.m_choices[f16_local2].params.extraParams.animationState, {
			material = f16_local0.stickLayoutSelector.m_choices[f16_local2].params.extraParams.settings.leftStickImage
		})
		f16_local0.stickLayoutImageContainer.rightStickImage:registerAnimationState(f16_local0.stickLayoutSelector.m_choices[f16_local2].params.extraParams.animationState, {
			material = f16_local0.stickLayoutSelector.m_choices[f16_local2].params.extraParams.settings.rightStickImage
		})
	end
	f16_local0:addElement(f16_local0.stickLayoutImageContainer)
	f16_local0.stickLayoutImageContainer:beginAnimation("fade_in", f16_local0.fadeInTime, false, false)
	f16_local0.stickLayoutImageContainer:setAlpha(1)
	f16_local0.stickLayoutSelector:processEvent({
		name = "gain_focus"
	})
	if CoD.isWIIU then
		CoD.WiiUControllerSettings.SetCloseIfControllerChanges(f16_local0)
	end
	return f16_local0
end
