...
CoD.Options.AddTwitchButton = function (f33_arg0, f33_arg1)
	if CoD.Twitch.IsRegistered(f33_arg1) then
		f33_arg0.twitchAccountButton = f33_arg0.buttonList:addButton(Engine.Localize("MENU_UNLINK_FROM_TWITCH_CAPS"), Engine.Localize("MENU_LINK_TWITCH_DESC"))
		f33_arg0.twitchAccountButton:setActionEventName("twitch_connect")
	end
end

CoD.Options.OpenTwitchConnect = function (f34_arg0, f34_arg1)
	if CoD.Twitch.IsRegistered(f34_arg1.controller) then
		f34_arg0:openPopup("Twitch_UnRegister", f34_arg1.controller)
	end
end

CoD.Options.TwitchConnectComplete = function (f35_arg0, f35_arg1)
	if f35_arg0.twitchAccountButton then
		f35_arg0.twitchAccountButton:close()
		f35_arg0.buttonList:processEvent({
			name = "gain_focus",
...
CoD.Options.BumperControlOverride = function (f37_arg0, f37_arg1)
	if LUI.UIElement.handleGamepadButton(f37_arg0, f37_arg1) == true then
		return true
	end
	local f37_local0 = nil
	if f37_arg1.down == true then
		if f37_arg1.button == "shoulderr" then
			f37_local0 = 1
		elseif f37_arg1.button == "shoulderl" then
			f37_local0 = -1
		end
	end
	if f37_local0 ~= nil and f37_arg0.m_currentItem ~= nil then
		f37_arg0:scrollToItem(f37_arg0.m_currentItem + f37_local0, f37_arg0.m_scrollTime)
	end
end

CoD.Options.SetupTabManager = function (f38_arg0, f38_arg1)
	local f38_local0 = CoD.Menu.TitleHeight + CoD.MFTabManager.TabHeight + 15
	local f38_local1 = CoD.ButtonPrompt.Height
	local Widget = LUI.UIElement.new()
	Widget:setLeftRight(true, true, 0, 0)
	Widget:setTopBottom(true, true, f38_local0, -f38_local1)
	f38_arg0:addElement(Widget)
	local f38_local3 = CoD.Menu.TitleHeight + 15
	local f38_local4 = CoD.MFTabManager.new(Widget, nil, nil, CoD.BOIIOrange)
	if f38_arg1 then
		f38_local4:setLeftRight(false, false, -f38_arg1 / 2, f38_arg1 / 2)
	else
		f38_local4:setLeftRight(true, true, 0, 0)
	end
	f38_local4:setTopBottom(true, false, f38_local3, f38_local3 + CoD.MFTabManager.TabHeight)
	f38_local4:setTabAlignment(LUI.Alignment.Center)
	f38_local4:setTabSpacing(20)
	f38_arg0.tabManager = f38_local4
	f38_arg0:addElement(f38_local4)
	return f38_local4
end

CoD.Options.CreateButtonList = function ()
	local f39_local0 = CoD.Options.ButtonListWidth
	local f39_local1 = 20
	local f39_local2 = CoD.ButtonList.new()
	if CoD.isSinglePlayer then
		f39_local2:setLeftRight(false, false, -f39_local0 / 2, f39_local0 / 2)
		f39_local2:setTopBottom(true, true, f39_local1, 0)
	else
		f39_local2:setLeftRight(true, false, 0, f39_local0)
		f39_local2:setTopBottom(true, true, f39_local1, 0)
		f39_local2.hintText:setLeftRight(true, false, 0, 800)
	end
	f39_local2.addHardwareProfileLeftRightSelector = CoD.Options.AddHardwareProfileLeftRightSelector
	f39_local2.addHardwareProfileLeftRightSlider = CoD.Options.AddHardwareProfileLeftRightSlider
	f39_local2.addLeftRightSelector = CoD.Options.AddLeftRightSelector
	f39_local2.addVoiceMeter = CoD.Options.AddVoiceMeter
	f39_local2.m_selectors = {}
	return f39_local2
end

CoD.Options.AddOptionCategories = function (f40_arg0)
	if UIExpression.IsInGame() == 0 then
		f40_arg0.systemInfoButton = CoD.ButtonPrompt.new("select", Engine.Localize("MENU_SYSTEM_INFO_CAPS"), f40_arg0, "open_system_info", nil, nil, nil, nil, "S")
		f40_arg0:addRightButtonPrompt(f40_arg0.systemInfoButton)
	end
	local f40_local0, f40_local1 = nil
	local f40_local2 = CoD.ButtonList.new()
	if UIExpression.IsInGame() == 0 then
		local f40_local3 = 50
		local f40_local4 = 30
		local f40_local5 = 300
		local f40_local6 = 2 * (f40_local3 + f40_local4) - f40_local4
		f40_local2:setLeftRight(false, false, -f40_local5 / 2, f40_local5 / 2)
		f40_local2:setTopBottom(false, false, -f40_local6 / 2, f40_local6 / 2 + 100)
		f40_local2:setSpacing(f40_local4)
		f40_local0 = f40_local2:addNavButton(Engine.Localize("MENU_SETTINGS_CAPS"), "open_settings")
		f40_local1 = f40_local2:addNavButton(Engine.Localize("MENU_CONTROLS_CAPS"), "open_controls")
		if not CoD.isSinglePlayer then
			f40_local0.brackets:close()
			f40_local1.brackets:close()
		end
	else
		if CoD.isSinglePlayer then
			f40_local2:setLeftRight(false, false, -CoD.ObjectiveInfoMenu.ElementWidth - CoD.ObjectiveInfoMenu.ElementSpacing / 2, -CoD.ObjectiveInfoMenu.ElementSpacing / 2)
			f40_local2:setTopBottom(true, true, CoD.ObjectiveInfoMenu.Pause_ButtonsTopAnchor, 0)
		else
			f40_local2:setLeftRight(true, false, 0, CoD.ButtonList.DefaultWidth)
			f40_local2:setTopBottom(true, true, CoD.Menu.TitleHeight + 50, 0)
		end
		if not CoD.isMultiplayer then
			f40_local2:setButtonBackingAnimationState({
				leftAnchor = true,
				rightAnchor = true,
				left = -5,
				right = 0,
		...
		f40_local0 = f40_local2:addButton(Engine.Localize("MENU_SETTINGS_CAPS"))
		f40_local0:setActionEventName("open_settings")
		f40_local1 = f40_local2:addButton(Engine.Localize("MENU_CONTROLS_CAPS"))
		f40_local1:setActionEventName("open_controls")
	end
	...
	if CoD.isSinglePlayer == true and Engine.IsMenuLevel() == true then
		f40_arg0:setPreviousMenu("CampaignMenu")
	end
	Engine.SyncHardwareProfileWithDvars()
end

LUI.createMenu.OptionsMenu = function (f41_arg0)
	local f41_local0 = nil
	if UIExpression.IsInGame() == 1 then
		f41_local0 = CoD.InGameMenu.New("OptionsMenu", f41_arg0, Engine.Localize("MENU_OPTIONS_CAPS"))
		if CoD.isSinglePlayer == true then
			f41_local0:setPreviousMenu("ObjectiveInfoMenu")
		elseif UIExpression.IsDemoPlaying(f41_arg0) ~= 0 then
			f41_local0:setPreviousMenu("Demo_InGame")
		else
			f41_local0:setPreviousMenu("class")
		end
	else
		f41_local0 = CoD.Menu.New("OptionsMenu")
		f41_local0.anyControllerAllowed = true
		f41_local0:addTitle(Engine.Localize("MENU_OPTIONS_CAPS"), LUI.Alignment.Center)
		if CoD.isSinglePlayer == false then
			f41_local0:addLargePopupBackground()
		end
	end
	if CoD.isSinglePlayer == true then
		Engine.SendMenuResponse(f41_arg0, "luisystem", "modal_start")
	end
	f41_local0:registerEventHandler("button_prompt_back", CoD.Options.Back)
	f41_local0:registerEventHandler("apply_changes", CoD.Options.ApplyChanges)
	f41_local0:registerEventHandler("open_controls", CoD.Options.OpenControls)
	f41_local0:registerEventHandler("open_settings", CoD.Options.OpenSettings)
	f41_local0:registerEventHandler("open_system_info", CoD.Options.OpenSystemInfo)
	f41_local0:registerEventHandler("is_common_loaded", CoD.Options.IsCommonLoaded)
	f41_local0:addSelectButton()
	f41_local0:addBackButton()
	CoD.Options.AddOptionCategories(f41_local0)
	return f41_local0
end
