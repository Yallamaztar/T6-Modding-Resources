...
require("T6.LobbyPanes")
require("T6.LobbyPlayerLists")
require("T6.PanelManager")
require("T6.UnifiedFriends")
require("T6.Menus.MTXPurchasePopup")
require("T6.Menus.PartyPrivacyPopup")
require("T6.Menus.LiveStream")
require("T6.Menus.TexturePackPopup")
if CoD.isZombie == false then
	require("T6.Menus.YouTube")
	require("T6.Menus.Twitter")
	require("T6.Menus.Twitch")
elseif CoD.isZombie and CoD.isPC then
	require("T6.Menus.YouTube")
end
CoD.Lobby = {}
local f0_local0 = {
...
CoD.Lobby.OverviewPane_PlayerSelected = function (f1_arg0, f1_arg1)
	if CoD.isZombie == true then
		if UIExpression.GetXUID(f1_arg0.controller) == f1_arg1.playerXuid then
...
CoD.Lobby.ButtonListGainFocus = function (f5_arg0, f5_arg1)
	...
	LUI.UIVerticalList.gainFocus(f5_arg0)
end
...
CoD.Lobby.PlayerSelected = function (f12_arg0, f12_arg1)
	if UIExpression.IsGuest(f12_arg1.controller) == 1 then
...
CoD.Lobby.New = function (LobbyType, LocalClientIndex, f15_arg2, f15_arg3, f15_arg4, f15_arg5)
	local f15_local0 = CoD.Menu.New(LobbyType)
	...
	f15_local0.panelManager = CoD.PanelManager.New(UIExpression.GetPrimaryController(), f15_local0.width, f15_local0.height)
	...
	f15_local0.lobbyPane:addPanelElements(f15_arg4, UIExpression.DvarInt(LocalClientIndex, "party_maxlocalplayers"), f15_arg2, f15_arg5)
	...
	if not CoD.isSinglePlayer == true and Engine.GameModeIsMode(CoD.GAMEMODE_LOCAL_SPLITSCREEN) == false and UIExpression.SessionMode_IsSystemlinkGame() == 0 then
	...
	if ShowGlobe then
		if CoD.isZombie == true then
			if Engine.GameModeIsMode(CoD.GAMEMODE_LOCAL_SPLITSCREEN) == false and UIExpression.SessionMode_IsSystemlinkGame() ~= 1 then
				ShowGlobe()
			end
		else
			ShowGlobe()
...
CoD.Lobby.OpenSignOutPopup = function (f18_arg0, f18_arg1)
	if CoD.isPS3 or CoD.isWIIU then
		if UIExpression.IsGuest(f18_arg1.controller) == 1 and not CoD.isZombie then
			...
			return true
		elseif UIExpression.IsSubUser(f18_arg1.controller) == 1 then
			local f18_local0 = f18_arg0:openPopup("SignOut", f18_arg1.controller)
			f18_local0:addButtons()
...
CoD.Lobby.OpenAfterActionReportIfNeeded = function (f19_arg0, f19_arg1)
	if f19_arg0.menuName == "PublicGameLobby" or f19_arg0.menuName == "LeaguePlayPartyLobby" then
		local f19_local0 = Engine.GetPlayerStats(f19_arg1)
		...
		local f19_local4 = UIExpression.IsStableStatsBufferInitialized() == 1
...
CoD.Lobby.OpenMenu = function (f20_arg0, f20_arg1)
	...
	if CoD.isZombie == true then
		f20_local1 = UIExpression.DvarString(f20_arg0.controller, "ui_zm_outromovie")
		Engine.SetDvar("ui_zm_outromovie", "")
		if f20_local1 ~= "" then
			CoD.FileAction.CinematicId = Engine.StartLoadingCinematic(f20_local1, true)
			f20_local0:openPopup("Cinematic_Player", f20_arg1.controller)
		end
	end
	if f20_arg0.menuName == "MainMenu" and CoD.isOnlineGame() == true then
	...
	CoD.Lobby.OpenAfterActionReportIfNeeded(f20_local0, f20_arg1.controller)
end
...
CoD.Lobby.CreateInfoPane = function ()
	local f26_local0 = LUI.UIContainer.new()
	f26_local0.backgroundContainer = LUI.UIElement.new({
		left = 0,
	...
	f26_local0:addElement(f26_local0.backgroundContainer)
	f26_local0.background = LUI.UIImage.new({
		left = 0,
		top = 0,
		right = 0,
		bottom = 0,
	...
	f26_local0.backgroundContainer:addElement(f26_local0.background)
	f26_local0.backgroundShadow = CoD.EdgeShadow.new("left", false)
	f26_local0.backgroundContainer:addElement(f26_local0.backgroundShadow)
	f26_local0.titleElement = LUI.UIText.new({
		left = 0,
		top = 0,
		right = 0,
	...
	f26_local0:addElement(f26_local0.titleElement)
	local f26_local1 = 200
	f26_local0.icon = LUI.UIImage.new({
		left = -f26_local1 / 2,
	...
	f26_local0:addElement(f26_local0.icon)
	f26_local0.description = LUI.UIText.new({
		left = 10,
	...
	f26_local0:addElement(f26_local0.description)
	return f26_local0
end

CoD.Lobby.AddLivestreamButton = function (f27_arg0, f27_arg1, f27_arg2)
	if UIExpression.DvarBool(nil, "webm_encUiEnabled") == 1 then
...
CoD.Lobby.ToggleLivestream = function (f30_arg0, f30_arg1)
	...
	if Engine.IsFeatureBanned(CoD.FEATURE_BAN_LIVE_STREAMING) then
		...
		return 
	elseif UIExpression.DvarBool(nil, "webm_encEliteRequired") == 1 and not Engine.IsPlayerEliteRegistered(f30_arg1.controller) then
...
CoD.Lobby.ButtonPromptFriendsMenu = function (f32_arg0, f32_arg1)
	if UIExpression.IsGuest(f32_arg1.controller) == 1 then
		f32_arg0:openPopup("popup_guest_contentrestricted", f32_arg1.controller)
		return 
	elseif not CoD.isPS3 or UIExpression.IsSubUser(f32_arg1.controller) ~= 1 then
...
CoD.Lobby.ButtonPromptPartyPrivacy = function (f33_arg0, f33_arg1)
	if UIExpression.IsGuest(f33_arg1.controller) == 1 then
...
CoD.Lobby.ConfirmLeave = function (f34_arg0, f34_arg1, f34_arg2, f34_arg3, f34_arg4)
	if f34_arg4 ~= nil then
		local f34_local0 = f34_arg0:openPopup("ConfirmLeave", f34_arg1, f34_arg4)
		for f34_local4, f34_local5 in pairs(f34_arg4.params) do
			f34_arg0:registerEventHandler(f34_local5.leaveEvent, f34_local5.leaveHandler)
		end
	else
		local f34_local0 = f34_arg0:openPopup("ConfirmLeave", f34_arg1)
		f34_local0:setConfirmEvent(f34_arg2)
		f34_arg0:registerEventHandler(f34_arg2, f34_arg3)
	end
end

CoD.Lobby.ConfirmLeaveGameLobby = function (f35_arg0, f35_arg1)
	local f35_local0 = {
		params = {}
	}
	if Engine.GameModeIsMode(CoD.GAMEMODE_LOCAL_SPLITSCREEN) == true or Engine.SessionModeIsMode(CoD.SESSIONMODE_SYSTEMLINK) == true then
		f35_local0.titleText = Engine.Localize("MENU_LEAVE_LOBBY_TITLE")
		table.insert(f35_local0.params, {
			leaveHandler = f35_arg1.leaveLobbyHandler,
			leaveEvent = "confirm_leave",
			leaveText = Engine.Localize("MPUI_YES"),
			debugHelper = "You're not in local game"
		})
		CoD.Lobby.ConfirmLeave(f35_arg0, f35_arg1.controller, "confirm_leave", f35_arg1.leaveLobbyHandler, f35_local0)
		return 
	elseif UIExpression.AloneInPartyIgnoreSplitscreen(f35_arg1.controller, 1) == 1 and UIExpression.InPrivateParty(f35_arg1.controller) == 1 then
		f35_local0.titleText = Engine.Localize("MENU_LEAVE_LOBBY_TITLE")
		table.insert(f35_local0.params, {
			leaveHandler = f35_arg1.leaveLobbyHandler,
			leaveEvent = "confirm_leave",
		...
		CoD.Lobby.ConfirmLeave(f35_arg0, f35_arg1.controller, "confirm_leave", f35_arg1.leaveLobbyHandler, f35_local0)
		return 
	elseif UIExpression.InPrivateParty(f35_arg1.controller) == 1 then
		if UIExpression.PrivatePartyHost(f35_arg1.controller) == 1 then
			f35_local0.titleText = Engine.Localize("MENU_LEAVE_LOBBY_TITLE")
			f35_local0.messageText = Engine.Localize("MENU_LEAVE_LOBBY_HOST_WARNING")
			table.insert(f35_local0.params, {
				leaveHandler = f35_arg1.leaveLobbyHandler,
			...
			f35_local0.titleText = Engine.Localize("MENU_LEAVE_LOBBY_TITLE")
			if UIExpression.PrivatePartyHostInLobby(f35_arg1.controller) == 1 then
				f35_local0.messageText = Engine.Localize("MENU_LEAVE_LOBBY_CLIENT_WARNING")
				table.insert(f35_local0.params, {
					leaveHandler = f35_arg1.leaveLobbyHandler,
					leaveEvent = "confirm_leave",
...
CoD.Lobby.PlayerListRowSelected = function (f36_arg0, f36_arg1)
	f36_arg0:processEvent({
		name = "player_selected",
		playerName = f36_arg1.playerName,
		playerXuid = f36_arg1.playerXuid,
		leagueTeamID = f36_arg1.leagueTeamID,
		leagueTeamMemberCount = f36_arg1.leagueTeamMemberCount,
		controller = f36_arg1.controller,
		joinable = f36_arg1.joinable
	})
end

CoD.Lobby.PlayerListRowDeselected = function (f37_arg0, f37_arg1)
	f37_arg0:processEvent({
		name = "player_deselected",
		playerName = f37_arg1.playerName,
		playerXuid = f37_arg1.playerXuid,
		controller = f37_arg1.controller
	})
end

CoD.Lobby.UpdatePanelFunctions = function (f38_arg0)
	if f38_arg0.buttonPane ~= nil then
		f38_arg0.buttonPane.addPanelElements = f38_arg0.addButtonPaneElements
		f38_arg0.buttonPane.populatePanelElements = f38_arg0.populateButtonPaneElements
	end
	if f38_arg0.lobbyPane ~= nil then
		f38_arg0.lobbyPane.addPanelElements = f38_arg0.addLobbyPaneElements
		f38_arg0.lobbyPane.populatePanelElements = f38_arg0.populateLobbyPaneElements
	end
	if f38_arg0.overviewPane ~= nil then
		f38_arg0.overviewPane.addPanelElements = f38_arg0.addOverviewPaneElements
		f38_arg0.overviewPane.populatePanelElements = f38_arg0.populateOverviewPaneElements
	end
	if f38_arg0.statsPane ~= nil then
		f38_arg0.statsPane.addPanelElements = f38_arg0.addStatsPaneElements
		f38_arg0.statsPane.populatePanelElements = f38_arg0.populateStatsPaneElements
	end
	if f38_arg0.survivalStatsPane ~= nil then
		f38_arg0.survivalStatsPane.addPanelElements = f38_arg0.addStatsPaneElements
		f38_arg0.survivalStatsPane.populatePanelElements = f38_arg0.populateSurvivalStatsPaneElements
	end
	if f38_arg0.encountersStatsPane ~= nil then
		f38_arg0.encountersStatsPane.addPanelElements = f38_arg0.addStatsPaneElements
		f38_arg0.encountersStatsPane.populatePanelElements = f38_arg0.populateEncountersStatsPaneElements
	end
end
