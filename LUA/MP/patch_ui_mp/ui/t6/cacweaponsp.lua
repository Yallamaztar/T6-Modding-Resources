...
CoD.CACPrimaryWeapons.AddWeaponGroupCarousel = function (f12_arg0, f12_arg1, f12_arg2, f12_arg3, f12_arg4, f12_arg5)
	...
	for f12_local10, f12_local11 in ipairs(f12_local2) do
		if f12_local11 ~= f12_arg3 and Engine.GetLoadoutSlotForItem(f12_local11) == f12_local0 then
			local f12_local9 = f12_local1:addNewCard()
			f12_local9.weaponItemIndex = f12_local11
			f12_local9.iconMaterial = RegisterMaterial(UIExpression.GetItemImage(nil, f12_local11) .. "_big")
			f12_local9.icon = LUI.UIImage.new({
				leftAnchor = true,
				rightAnchor = true,
				left = 10,
				right = -10,
				...
				green = 1,
				blue = 1,
				material = f12_local9.iconMaterial
			})
			f12_local9:addElement(f12_local9.icon)
			f12_local9.title = LUI.UIText.new({
				leftAnchor = true,
				rightAnchor = true,
				left = 5,
				right = 0,
			...
			f12_local9.titleText = Engine.Localize(UIExpression.GetItemName(nil, f12_local11))
			f12_local9.title:setText(f12_local9.titleText)
			f12_local9.title:setAlignment(LUI.Alignment.Left)
			f12_local9.title:registerAnimationState("button_over", {
				red = CoD.BOIIOrange.r,
				green = CoD.BOIIOrange.g,
				blue = CoD.BOIIOrange.b
			})
			LUI.UIButton.SetupElement(f12_local9.title)
			f12_local9:addElement(f12_local9.title)
			if UIExpression.IsItemLocked(f12_arg1, f12_local11) == 1 then
				f12_local9.lockedIcon = CoD.CAC.SetLockIcon(f12_local9, 64, f12_local4)
				f12_local9.isLocked = true
				f12_local9.icon:setAlpha(0)
				f12_local9:setAlpha(0.5)
				f12_local9.title:registerAnimationState("button_over", {
					red = 1,
					green = 1,
					blue = 1
				})
			elseif UIExpression.IsItemPurchased(f12_arg1, f12_local11) == 0 then
				f12_local9.tokenIcon = CoD.CAC.SetTokenIcon(f12_local9, f12_local5)
				f12_local9.icon:setAlpha(0.25)
			elseif Engine.IsItemIndexRestricted(f12_local11) then
				f12_local9.restrictedIcon = CoD.CAC.SetLockIcon(f12_local9, 32, f12_local3)
			end
			if Engine.IsItemNew(f12_arg1, f12_local11) then
				CoD.CAC.SetNewIcon(f12_local9, true)
			end
			if f12_local9.border then
				f12_local9.border:close()
			end
			if f12_local9.highlightedborder then
				f12_local9.highlightedborder:close()
			end
			CoD.ContentGridButton.SetupButtonImages(f12_local9, CoD.GrenadeGridButton.glowBackColor, CoD.GrenadeGridButton.glowFrontColor)
			f12_local9:setActionEventName("weapon_chosen")
			if f12_local11 == f12_arg4 then
...
CoD.CACPrimaryWeapons.WeaponPrestige = function (f14_arg0, f14_arg1)
	if Engine.IsFeatureBanned(CoD.FEATURE_BAN_PRESTIGE) then
		Engine.ExecNow(f14_arg1.controller, "banCheck " .. CoD.FEATURE_BAN_PRESTIGE)
		return 
	elseif Engine.IsFeatureBanned(CoD.FEATURE_BAN_PRESTIGE_EXTRAS) then
		Engine.ExecNow(f14_arg1.controller, "banCheck " .. CoD.FEATURE_BAN_PRESTIGE_EXTRAS)
		return 
	else
...
CoD.CACPrimaryWeapons.WeaponChosen = function (f15_arg0, f15_arg1)
	...
	Engine.SetItemAsOld(f15_arg1.controller, f15_arg1.button.weaponItemIndex)
